...
export default function Dashboard({ project, tasks }: DashboardProps) {
  //pantalla finish project resumen final
        ...
        <Card>
          <CardHeader className='flex flex-row items-center justify-between'>
            ...
            <div className='flex gap-2'>
              <Link href={`/software/project/${project.id}/tasks/new`}>
                <Button>
            ...
            <div className='space-y-4'>
              ...
              {!allTasksCompleted && totalTasks > 0 && (
                <div className="text-sm text-muted-foreground mt-2">
                  Complete all tasks to finish this project ({completedTasks} of {totalTasks} completed)
                </div>
              )}
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
                ...
                <div className='grid gap-2'>
                  {backlogTasks.length > 0 ? (
                    backlogTasks.map((task) => (
                      <Link key={task.id} href={`${project.id}/tasks/${task.id}/edit`}>
                    ...
                    doneTasks.map((task) => (
                      <Link key={task.id} href={`${project.id}/tasks/${task.id}/edit`}>
                        <Card key={task.id} className='p-4'>
                          <div className='flex items-center justify-between'>
                            <div className='text-sm font-medium'>{task.name}</div>
                            <Badge variant={getPriorityVariant(task?.priority ?? null)}>{task.priority}</Badge>
                          </div>
                          <div className='mt-2 text-xs text-muted-foreground'>{task.estimated_hours} hours</div>
                        </Card>
                      </Link>
            ...
            <div className='mt-4 flex justify-end'>
              <Link href={`/software/project/${project.id}/tasks`}>
        ...
        <Card>
          ...
          <CardContent>
            <div className='flex gap-3 flex-wrap'>
              ...
              <Link href={`/software/project/${project.id}/edit`}>
                <Button className='w-full justify-start' variant='outline'>
                  <IconEditCircle className='mr-2 h-4 w-4' />
                  Edit Project Details
                </Button>
              </Link>
              {/* <Link href={`/software/project/${project.id}/sprints`}>
                <Button className="w-full justify-start" variant="outline">
                  Manage Sprints
                </Button>
              </Link> */}
              {/* {tasks.length > 0 && completedTasks === tasks.length ? (
                <Link href={`/project/${project.id}/finish`}>
                  <Button className='w-full justify-start mt-4' variant='default'>
                    <CheckCircle className='mr-2 h-4 w-4' />
                    Finish Project
                  </Button>
                </Link>
              ) : (
                <Button className='w-full justify-start mt-4' variant='default' disabled>
                  <CheckCircle className='mr-2 h-4 w-4' />
                  Finish Project
                  <span className='ml-2 text-xs opacity-70'>(Complete all tasks first)</span>
                </Button>
              )} */}
            </div>
          </CardContent>
        ...
        <Card>
          ...
          <CardContent>
            <div className='space-y-4'>
              ...
              <div>
                <div className='text-sm font-medium text-muted-foreground'>End Date (est.)</div>
                <div>{endDate}</div>
              </div>
              <div>
                  ...
                  <div className='flex-1'>
                    <div className='h-2 w-full bg-muted rounded-full'>
                      <div
                        ...
                        style={{ width: totalTasks > 0 ? `${(count / totalTasks) * 100}%` : "0%" }}></div>
                    </div>
                  </div>
